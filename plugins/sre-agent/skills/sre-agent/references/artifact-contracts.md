...
Bootstrap also stamps the CAPABILITY MAP before Scout: the pinned capability baseline bound
by content hash, the presence stamp (which capabilities the host exposes this run), the live
model intersection, incident relevance, and gaps recorded. It does not restate the baseline's
rows. It also labels the per-service reusable-guidance shelf when a capability can resolve service docs, runbooks, failure modes, observability pointers, and curated guidance content for Scout coverage scans. For each key failing signal, the map should name any catalog/runbook-known adjacent producer/dependency observability classes likely to consume the failing units' identity/time/scope/operation pivots, including the dependency's own logs and exported metrics, to aim later discovery probes; this list is non-authoritative, and omission never closes a candidate or licenses a reachability cap under the reachability floor. For each CRITICAL-EVIDENCE-PATH
capability, include one ACCESS STATUS line:
- `status`: `confirmed` | `unconfirmed-nondiagnostic(self-misuse | wrong-target | auth-cold-start | probe-defect)` | `blocked(hard-absent | denied-after-valid-auth)` | `not-exposed` (the host staged no handle for it this run — a presence fact, never a block and never a `denied`/`discovery_exhausted` receipt). `hard-absent` requires a STAGED handle whose canonical recipe returned missing-command; an unset handle never enters the probe path and never becomes `blocked(*)`. A `not-exposed` capability is not a fit alternate and cannot satisfy `none-named` — see [grading-rubric.md](grading-rubric.md) § blocked-unreachable. These are the intake-time subset of the run access ledger's states, whose canonical home is [run-store.md](run-store.md) § Access ledger; `advertised-unverified`, `candidate-failure`, and `open-gap` additionally appear there as the run progresses.
- `target_status`: `deferred-to-consumer` unless the coordinator itself confirmed a specific source; entrypoint confirmation never implies target reachability
...
The Grader manually selects the **one most decisive verification query** by `runId`
(from a deterministic candidate menu) into `5_grader/decisive-queries.toon`, and the
posting capability resolves that runId to a deep-link mechanically — so the link never
depends on a specialist copying a URL. Manual selection is the only relevance signal;
query text, labels, identifiers, row counts, and diagnostics never establish relevance
by themselves. When the selected query's `observation_ref` proves one visible claim or
Failure path node, attach `[query]` there and include the observed result; do not duplicate
it. Render a fallback **Evidence Kit** only when the selected query cannot map to one exact
visible claim. `6_report/evidence-kit.md` and `6_report/incident-update-with-kit.md` are
...
Also produces `5_grader/decisive-queries.toon`, with one compact frame for the manually selected query.
...
`6_report/investigation-report.md` and any derived incident update are read by a
tired on-call engineer under time pressure. Write them in ASD-STE100 Simplified
Technical English and follow the Microsoft Writing Style Guide. Plain language
raises the honesty bar and never licenses an uncited cause, a dropped confidence
grade, or a softened gap.

- Lead with the answer you can support: the first prose sentence says what broke
  and what to do next.
- Use one idea per sentence. Use active voice and name the actor.
- Prefer a verb to a noun built from a verb: "we could not read the failure
  reason", not "reason-record acquisition was unresolved".
- Reuse the service's own words. Give every number a unit and a plain meaning,
  such as "3 of 50 calculations failed (6%)". Write link text a human can read,
  never a path.
- Treat admitted operational values as answer content, not footnotes. State the
  actors, inputs, locations, contexts, times, states, failing set, outcome, shared
  values, healthy comparison, and tested scope that an operator needs to identify
  the owner and act. Put each evidence citation beside its claim. If a reader must
  open a cited path to learn a material value, the output is incomplete.
- State a gap as missing evidence or a missing action. Not "producer-semantic
  mis-emission is not fully excluded", but "we cannot rule out that the metric
  reported a failure that did not happen".

Internal coordination artifacts are exempt.
...
5. **Action and uncertainty** — keep one owner-routed next action prominent. Name
   ...
   check.
6. **Evidence Kit** — fallback only when the one canonical capability-resolved decisive query cannot map to one
   exact visible claim; place it after the optional path and never duplicate an inline query link.
...
Evidence pointers use owner-resolvable source terms. On each proving visible claim or
optional Failure path node, retain every relevant anchor already carried by its evidence lineage. An
...
OBS/evidence row and place it with the observed result on the exact visible claim or
Failure path node it proves. Do not
...
- Add an internal **Manual Investigation Kit** when a decisive discriminator genuinely requires
  ...
  7. Keep the selected Kusto link on its exact proving claim or Failure path node when safe; use the
     fallback **Evidence Kit** or Technical details otherwise. Never put it in `OCE next checks`.
