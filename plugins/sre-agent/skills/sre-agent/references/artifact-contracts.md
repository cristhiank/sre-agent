...
The post's clickable-evidence surface is the **curated Manual Investigation Kit**: the Grader selects the
...
- Add a **Known / ongoing issue note** when the run took the intake fast-lane and the confirming probe returned CONFIRMED (see `SKILL.md` § Intake fast-lane and `fast-lane.md`): render the recurrence-family disposition — the recurrence ledger ref (or, for a Tier-2 self-derived discriminator, the recognized-recurrence label), the live confirming observation (the discriminator result for THIS incident), and the engineer next-step. It is a known-issue disposition, NOT a Grader verdict band, and never a `duplicate-of`/`canonical` claim. Its finalize is TIER-GATED by the fast-lane receipt's `discriminator_source`: `ledger` (Tier 1) may post live under the poster's authorization gate; `derived` (Tier 2) is composed into this report-only finalize ONLY, as the `report-only (tier-2 self-derived; human confirmation required)` outcome — a recognized-recurrence DRAFT for human confirmation that performs NO incident-system mutation, regardless of authorization or capability mode (`DRAFT` = this report-only finalize path, reusing `6_report/incident-update-with-kit.md`). See `subagents/poster.md` § Live incident-system posting (tier precondition) / § Verdict policy.
