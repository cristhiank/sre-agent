...
- `observed_identity{scenario, signal, owner, correlation_key}` — projected from an authoritative probe; each field non-null, or recorded `n/a-field-not-carried` with cited justification where the capability does not expose that field (distinct from a capability being `not-exposed` this run).
...
call_id) OR `discovery_exhausted` (required bounded
discovery sequence completed; capability-map or baseline absence may AIM that sequence but
never substitutes for completing it) — per `artifact-contracts.md` §`5_grader/`; there is no unattempted-but-blocked
...
>
> **Terminology:** *CAPABILITY MAP* (and any scout enumeration) — **run-stamped over a pinned baseline** ⇒ a **discovery artifact**; it aims and ranks probes, NEVER closes a candidate. Mere absence or silence from any map, baseline, or enumeration is never the required explicit negative. *Discovery artifact* — scout output, derived map, the pinned capability baseline, or any enumeration; routes and ranks, NEVER closes. KB/docs notes are orientation — neither class. A static capability baseline MAY be supplied; it is routing metadata and is explicitly NON-authoritative, so there is no inventory-based closer — supplied or derived. A capability the host did not expose this run is `not-exposed`: a presence fact, never a `denied` or `discovery_exhausted` receipt. A `not-exposed` capability is also NOT a fit alternate for consolidation — it cannot be probed, so it never enters the candidate set, never discharges the horizontal pivot, and never satisfies `none-named`; equally it never licenses a cap. When a lead's only remaining alternate is `not-exposed`, the lead settles **open-answerable — Proximate-only with the gap visible**, naming the missing host handle as the engineer's next step; it is never `natural-ceiling` or `blocked-unreachable`. Full field definitions: `artifact-contracts.md` §`5_grader/`.
>
