...
IMPORTANT: do not settle Inconclusive-blocked or a final Proximate-only while the discovery receipt's cross-source-pivot is incomplete (a probed source + observed result, or a terminal error after an authed probe — not a soft "unavailable"); see [artifact-contracts.md](artifact-contracts.md) §`5_grader/`. Spend at most one focused probe from the remaining follow-up budget; if no reachable in-hand-keyed source remains, that terminal IS the receipt — settle and name the engineer next step; do not loop.
