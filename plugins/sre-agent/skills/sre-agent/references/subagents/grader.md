...
Dead-end + engineer suggested step: (only when blocked-unreachable after cap-time falsifier check passed — grading-rubric.md § blocked-unreachable reachability floor)
...
artifact reads, and no prose recap. If the envelope expires, qualify/block the unaudited
