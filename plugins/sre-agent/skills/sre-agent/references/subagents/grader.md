...
- the candidate evidence-query menu for this run (each entry: `runId` + query summary + source), provided in your brief — the closed set you pick the kit's verification deep-links from (you reference queries by `runId`; you never run queries or handle URLs)
...
## Decisive verification queries (Manual Investigation Kit selection)

From the candidate evidence-query menu in your brief, pick the **1-3 queries that would let an OCE independently verify the root cause** and write `5_grader/decisive-queries.toon`, one selection per line as `<runId> | <why>`. The `why` is the OCE-facing label — what the query proves (e.g. the throttle↔failure correlation join, or the cap-constant-both-weeks check). Rules:
