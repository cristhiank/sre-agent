...
Evidence sits on distinct planes and a claim is answerable only by the plane that holds it: current source carries code and config, the per-service knowledge base describes those services, and the cross-service method layer carries generalized method. Match the plane to the claim before assigning the search.
