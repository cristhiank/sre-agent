...
**PASS:** every M artifact FILE is present (a `none-found`/gap note is content within that file about what was found — it is never a substitute for the file's existence); every P artifact is present iff its predicate outcome is true (gap record in `00-index/evidence-ledger.toon` required for inconclusive predicates), including `observability/dependency-sources.md` present iff P-dep-telemetry has any `promoted` or `open:escalated` outcome and absent only when all dependencies have terminal `rejected` ledger records carrying `none-found (searched scope)`; `topology/capability-flows.md` present iff P-capability-flows is true (≥1 named capability met the Phase-3 when-to-capture threshold) and absent when false (gap record required for inconclusive predicates); `flows-obligation-unmet=yes` in `@meta` is set when a named capability was detected but not captured this run (threshold met, capture incomplete) and cleared when captured or predicate no longer holds; every G artifact present or has a capability-gap note; every V artifact present for each passing promotion predicate; no extra transient or scratch artifacts in deliverable root; `kb/**/deep/*.toon` included in audit scope; file count and glob scope reported.
**FAIL:** any M artifact FILE absent; any P artifact present for a false/inconclusive predicate without a gap record; `observability/dependency-sources.md` absent when P-dep-telemetry has a `promoted` or `open:escalated` outcome, or present when all dependencies have terminal `rejected` ledger records carrying `none-found (searched scope)`; `topology/capability-flows.md` absent when P-capability-flows is true; `topology/capability-flows.md` present when P-capability-flows is false without a gap record; `flows-obligation-unmet` set in `@meta` without a matching capability name and gap reason; any G artifact absent without a capability-gap note; any V artifact missing for a true predicate; any transient/scratch artifact in the deliverable root; deep/*.toon excluded from audit scope.
...
**Scope:** `00-index/evidence-ledger.toon @meta` capability-map-summary; all KB artifacts using capability/live state words; per-row `restricted?` values in `observability/source-catalog.md` and `observability/dependency-sources.md`.  
**Procedure (a) — `@meta` capability state:** for every capability-map-summary entry and every live/capability state word in `@meta` blocks and KB artifacts: verify the value is in the canonical 3-item vocabulary (`references/reproducibility-contract.md §Live-capability vocabulary`): `disabled-by-scope | attempted-unreachable | reachable-snapshot`. For any `attempted-unreachable` entry: verify a probe-result is recorded (capability name, canonical target, auth-vs-absence result).  
**Procedure (b) — per-row `restricted?`:** for every `restricted?` cell in source-catalog and dependency-sources rows: verify (1) the value is in the per-row legal set `{no | yes-<reason> | UNVERIFIED-THIS-PASS}` (see `references/artifact-manifest.md §observability/source-catalog.md Hard rules`); (2) `disabled-by-scope` does not appear (it is PASS-scope only; forbidden as ROW scope); (3) any row where `restricted?=yes-<reason>` has a cited probe-denial receipt in the row or linked ledger record; (4) every corresponding `observability/restricted-sources.md` projection row has a non-empty `receipt-ref`.  
**PASS:** every `@meta` capability state is in the canonical 3-item vocabulary; every `attempted-unreachable` entry has a recorded bounded probe result; every per-row `restricted?` is in `{no | yes-<reason> | UNVERIFIED-THIS-PASS}`; no per-row `restricted?` contains `disabled-by-scope`; every `restricted?=yes-*` row has a cited probe-denial receipt; every `restricted-sources.md` projection row has a non-empty `receipt-ref`.  
**FAIL:** bare `unreachable` or `unknown` without probe proof; any non-canonical `@meta` capability state word; any `attempted-unreachable` entry without a probe-result record; any per-row `restricted?` outside `{no | yes-<reason> | UNVERIFIED-THIS-PASS}`; `disabled-by-scope` appearing as a per-row `restricted?` value; any `restricted?=yes-*` row lacking a cited probe-denial receipt; any `restricted-sources.md` row with an empty `receipt-ref`.
