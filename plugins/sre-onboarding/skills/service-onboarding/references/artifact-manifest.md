...
services/<service>/
  ...
  topology/
    ...
    capability-flows.md                     P  (P-capability-flows=threshold-met)
  observability/
    README.md                               M
    source-catalog.md                       M
...
Node/edge table. Edge `kind` enum: `observed-runtime-call | embedded-library | deploy/config-reference | test-only | docs-only | generated/unused-candidate`. Auth/identity as edge attributes. For `kind=embedded-library`, optional producer attribution edge attribute: `producer-service=<service>; package-key=<package-or-assembly>`, where `package-key` is the same canonical identity as the dependency's `topology/dependencies.md` `kind=library` row (one shared key for the cross-artifact join): the ecosystem-qualified, lower-cased package/assembly name without version — e.g. `nuget:contoso.telemetry`, `npm:@contoso/pkg`, `maven:com.contoso/lib`. Closed-world ban applies; absence claims must name searched scope.
...
### `topology/capability-flows.md` (P — mandatory iff P-capability-flows=threshold-met)

Named multi-hop pipeline and mutually-exclusive gated-path index. Captured ONLY when a named capability meets the when-to-capture threshold (see `references/workflow.md §Phase 3`); absent otherwise (capability-gap note in `00-index/evidence-ledger.toon`). Flows SUMMARIZE and INDEX; they NEVER re-describe or duplicate an edge row.

**Stable ID:** `flow.<capability-slug>` — slug rules in `references/reproducibility-contract.md`.

**Schema:** `flow-id | flow-kind {pipeline | capability-path} | capability | data-entity | paths[ path-id, selection-gate{kind, observable-at, values-per-path} (required iff >1 path), ordered-hops[ service/component, role {producer|transformer|broker|sink|reader}, seam {api|queue|table|dataset|export|job|event}, edge-ref (→ service-graph) | handoff-ref (→ data-flow-handoffs), telemetry-pivots ], terminal-artifact, failure-signatures ] | trust | last-verification | evidence-sources[] | status {verified|unverified|stale|flow-candidate} | owner`

**Pointer-only hops (anti-bloat):** every hop MUST carry an `edge-ref` (→ `topology/service-graph.md`) or `handoff-ref` (→ `topology/data-flow-handoffs.md`). A hop with no ref MUST be marked `flow-candidate` and the flow status is `flow-candidate`. Flows index; they NEVER re-describe or duplicate an edge row.

**Trust and freshness rollup:** `trust` = weakest-hop trust across all hops in the flow; `last-verification` = oldest-hop `last-live-verification`. Staleness surfaces mechanically.

**Anti-rot:** only capabilities present in `evidence-sources[]` (incident/RCA/doc/ticket/dashboard evidence) may become flows — no speculative paths; stale flows are retired or degraded to `stale`; no prose narrative except to disambiguate a selection gate.

**`flows-obligation-unmet`:** SET when a named multi-realization capability is detected AND the Phase-3 threshold is met but the flow cannot be fully captured in this run (emit `flows-obligation-unmet=yes` at `@meta` in `00-index/evidence-ledger.toon` naming the capability and gap reason); CLEAR when the flow is captured (`topology/capability-flows.md` present and current for that capability) or the threshold no longer holds.
...
Schema: `signal | source-kind | table/metric/log-name | join-keys | dimensions | recipe/query-pointer | discovery-source | caller/access-scope | last-live-verification(date or NEVER) | trust | grounding_type | restricted? | exact-verify-later-action | CONSUMER WARNING`.

Hard rules (gating done):

**Reachability scope separation (canonical home — pointers in `references/reproducibility-contract.md §Live-capability vocabulary` and `references/workflow.md §Phase 5`):** three scopes must not be conflated:
- **PASS scope** — capability-map-summary entries in `00-index/evidence-ledger.toon @meta` (canonical values: `disabled-by-scope | attempted-unreachable | reachable-snapshot` per `references/reproducibility-contract.md §Live-capability vocabulary`): metadata about THIS run only. `disabled-by-scope` here means the capability was deliberately excluded from this run by design.
- **ROW scope** — per-row `restricted?` is a fact about the SOURCE, not the run; it is receipt-backed (see receipt rule below). Legal values: `no` (reachable / not blocked); `yes-<reason>` (blocked / access-denied — requires a probe-denial receipt); `UNVERIFIED-THIS-PASS` (not probed in this pass with no prior verified value).
- **Propagation ban:** the token `disabled-by-scope` is legal ONLY at `@meta`; it is FORBIDDEN as a per-row `restricted?` value. A pass-level "no live mode" NEVER propagates into any row's `restricted?`.

**Receipt rule:** a per-row `restricted?=yes-<reason>` (any blocked/denied value) is valid ONLY when a probe-denial receipt exists — an actual canonical invocation (non-guessed target) that returned an auth/permission/missing-source/schema-absent error, cited in the row or linked ledger record. A documentation gap, a pass-level "no live mode," or `@meta.live-evidence-mode=disabled-by-scope` alone is NOT a valid basis for any `restricted?=yes-*` value.

**UNVERIFIED state:** when a pass does NOT probe a source AND no prior verified value exists, the ONLY legal output is `restricted?=UNVERIFIED-THIS-PASS`, `last-live-verification=NEVER`. Trust is CONDITIONAL on discovery method: `source-inferred/declared` when the source was discovered from code, config, or schema; `docs-only` when discovered from human docs only (this rule is the single canonical home; `references/workflow.md §Phase 5` defers here). The rendered `CONSUMER WARNING` column for this row MUST contain: "probe at investigation time; do NOT treat as blocked."

**Retention/idempotence (load-bearing):** a pass with no live access to a source MUST NOT overwrite a prior row's verified value — a prior `restricted?=yes-*` backed by a receipt, or a prior verified-reachable row with `last-live-verification=<date>`. Protected fields carried forward unchanged: `restricted?`, `last-live-verification`, `trust`, and the cited receipt reference. It MAY annotate `unverified-this-pass` as a sibling note in a separate field, never as the `restricted?` value. The only legal fresh-write to `restricted?` is `UNVERIFIED-THIS-PASS` when NO prior verified value exists. **Staleness ceiling:** a `verified` value older than the service's declared freshness window (or 5 consecutive runs without re-verification, whichever is shorter) becomes `stale-verify-pending` — still not-blocked, but flagged for re-probe in `exact-verify-later-action`; it is not indefinitely carried as `verified`. **Recency-precedence:** a sibling's reachable evidence may override a retained denial receipt ONLY if the sibling evidence is current-pass or timestamped newer than that receipt; a merely-retained (stale) `last-live-verification=<date>` does not override a this-pass receipt-backed denial.

**Physical-source identity** (the cross-file match key for reconciliation and projection — NOT a stable row ID): normalized `(source-kind + table/metric/log-name + cluster/db-or-endpoint-class + caller/access-scope)`. When `cluster/db-or-endpoint-class` is absent, degrade the key to `(source-kind + table/metric/log-name + single-scope)` and attach an explicit `single-scope` marker; do not over-match two rows whose `cluster/db-or-endpoint-class` fields are both blank.

**Same-run sibling reconciliation:** before a run closes, for every sibling artifact that references sources also cataloged here (e.g., `observability/dependency-sources.md` with a live sample), match rows on physical-source identity. If a sibling records the SAME PHYSICAL SOURCE as reachable (live-sample or `last-live-verification=<date>`), apply: (a) **scope-equivalence required** — the sibling's `caller/access-scope` must equal or be a documented superset of the denied row's `caller/access-scope`; (b) when scope is equivalent, any contradicting row with `restricted?=yes-*` or `restricted?=UNVERIFIED-THIS-PASS` MUST be corrected to verified-reachable, citing the sibling as evidence; (c) when scope is NOT equivalent, RETAIN the original row as a scope-qualified residual denial and raise a human-reconciliation Question noting the scope mismatch — do not force-clear; (d) a superseded `restricted-sources.md` entry is demoted/annotated (record a `superseded-by` note with the sibling evidence reference), not silently deleted.
...
**Derived projection only.** This file is generated solely from rows in `observability/source-catalog.md` (and `observability/dependency-sources.md`) whose `restricted?=yes-<reason>` carry a probe-denial receipt (see source-catalog hard rules above). Free-form authored entries are forbidden. An entry is superseded when a later pass or sibling artifact records the same physical source as reachable under an equivalent scope (same-run sibling reconciliation rule, above); superseded entries are demoted/annotated with a `superseded-by` note, not silently deleted. Consumers must not generalize a gate on one source to all sources.

Schema: `source | gate/rejection-observed | scope-tested | alternate-reachable-source? | consumer-note | receipt-ref`. `scope-tested` is populated from the `caller/access-scope` field of the originating source-catalog row. `receipt-ref` cites the ledger record ID or cited invocation carrying the probe-denial receipt; an entry without a `receipt-ref` is invalid.
