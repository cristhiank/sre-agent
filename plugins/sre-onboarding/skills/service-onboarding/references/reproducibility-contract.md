...
Canonical states for `capability-map-summary` entries in `00-index/evidence-ledger.toon @meta` and for any live/capability state recorded in KB artifacts at `@meta` scope. Any `@meta`-scope capability state not in this table is a FAIL in mechanical audit item I (a); per-row `restricted?` values operate under a separate ROW-scope value set — see `references/artifact-manifest.md §observability/source-catalog.md Hard rules` and audit item I (b).
...
**Three-scope separation:** canonical three-scope rule (PASS scope vs ROW scope), propagation ban, receipt rule, UNVERIFIED state, retention/idempotence, and sibling reconciliation: `references/artifact-manifest.md §observability/source-catalog.md Hard rules` (canonical home).
...
| `flow.<slug>` | capability flow (multi-hop pipeline or gated path) | normalized capability name |
...
### P-capability-flows — Capability-flows index

`topology/capability-flows.md` is mandatory iff ≥1 named capability meets the when-to-capture threshold: the capability appears in evidence-backed sources (incident/RCA/doc/ticket/dashboard) AND (a) has ≥2 mutually-exclusive realizations with an observable selection gate, OR (b) has a single realization spanning ≥3 hops with a persistent terminal artifact. Full threshold definition: `references/workflow.md §Phase 3`. A false predicate outcome requires a gap record in `00-index/evidence-ledger.toon`; the file is absent when no capability meets the threshold. `flows-obligation-unmet` SET/CLEAR lifecycle: `references/artifact-manifest.md §topology/capability-flows.md`.
