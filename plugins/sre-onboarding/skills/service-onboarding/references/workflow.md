...
- Enumerate the owned source scope from `sources.yaml` `owns` entries, resolve each source ID to `catalog/sources/<id>/worktrees/<sha>/` (SHA from the input/run lock), record SHAs, and classify deployable-binary vs library.
...
- Confirm source-inventory completeness.
...
- Produce the per-repo/per-source incident-material roll-up: `incident-material(yes/no) | trigger-category (if yes) | searched-scope`. Namespace bridge: each owned git-repo source maps 1:1 to `repo.<slug>` and `kb/<repo>/`; non-repo sources (for example `kind: ado-wiki`) map to `source.<slug>` and `kb/<source>/`. Excluded sources get a `non-material` ledger row with searched scope.
...
- Classify every candidate edge per the edge-`kind` enum in `references/artifact-manifest.md §topology/service-graph.md`. Emit one ledger record per classified edge.
...
- **Capability-flows (seam inventory; opt-in; default = leave in flat edge list):** while inventorying seams, flag any NAMED CAPABILITY (appears in an incident/RCA/doc/ticket/dashboard) that has (a) ≥2 mutually-exclusive realizations with an observable selection gate, OR (b) a single realization spanning ≥3 hops with a persistent terminal artifact. Such a named capability requires a first-class entry in `topology/capability-flows.md` (O); a flat edge list without path-selection logic for it is UNDER-MINED. If a named multi-realization capability is detected but cannot be fully captured in this run, emit `flows-obligation-unmet=yes` at `@meta` in `00-index/evidence-ledger.toon` with the capability name and gap reason. Schema, pointer-only-hop rule, and anti-rot: `references/artifact-manifest.md §topology/capability-flows.md`.
...
Apply `verification-and-evidence.md` rulebook. Re-resolve citations. Verify telemetry live when reachable; otherwise create source-catalog rows with `last live verification = NEVER`, `restricted?=UNVERIFIED-THIS-PASS` (never `yes-disabled-by-scope`), trust per discovery method (single rule: `references/artifact-manifest.md §observability/source-catalog.md Hard rules`), exact verify-later action, and a CONSUMER WARNING containing the anti-absence clause (source and join key filled) AND "probe at investigation time; do NOT treat as blocked" for `UNVERIFIED-THIS-PASS` rows. The pass-level `@meta.live-evidence-mode` value NEVER propagates into any row's `restricted?` column. Full three-scope separation, receipt rule, UNVERIFIED state, retention/idempotence, and same-run sibling reconciliation: `references/artifact-manifest.md §observability/source-catalog.md Hard rules`.
