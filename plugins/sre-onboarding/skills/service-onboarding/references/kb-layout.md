...
| `topology/` | where it runs + who calls whom | service-graph, endpoints/ports catalog, dependencies, blast-radius, deployable-unit coverage matrix, per-deployable-units, data-flow-handoffs (artifact-mediated handoffs), optionally capability-flows when Phase-3 threshold met (predicate-conditional — see `references/artifact-manifest.md §topology/capability-flows.md`) |
...
`topology/capability-flows.md`: Predicate-conditional (P — required when Phase-3 threshold met). Present only when ≥1 named capability meets the when-to-capture threshold (`references/workflow.md §Phase 3`). Pointer-only hops (every hop carries `edge-ref` → service-graph or `handoff-ref` → data-flow-handoffs); no edge re-description. `flow.<slug>` stable IDs. Full schema, predicate definition, and anti-rot rules: `references/artifact-manifest.md §topology/capability-flows.md`.
...
`observability/restricted-sources.md`: derived projection only — generated from receipt-backed `restricted?=yes-*` rows; free-form authored entries are forbidden; see `references/artifact-manifest.md §observability/restricted-sources.md` for the full rule. Consumers must not generalize a gate on one source to all sources.
